...
**Design decision D-4 (UNIFY rule dispatch at resolution).** Correction to my first draft (adversarial review): scoring *does* run live — `resolve_and_score_weather_beliefs` and `resolve_and_score_funding_beliefs` (daemon.rs) compute the score in-process at the daily boundary. The real gaps are: (a) they **hand-roll** the score (e.g. `score_bucket`/pinball math) instead of dispatching the `ScoringRule` TRAIT (`scoring.rs` BrierRule/CrpsPinballRule), so adding Log/RPS means editing each resolver; (b) it is **forked per producer** (`_weather_` vs `_funding_` vs the persona path) rather than ONE data-type-dispatched path — which is what blocks the fair head-to-head; (c) **binary synthesis beliefs have no resolver at all**. D-4 therefore = a single `score_resolved_beliefs(scope)` that selects the rule by `PredictiveKind` (data), calls `rule.score(...)`, and persists via `resolve_and_score` — replacing the three forked hand-rolled resolvers. (This makes G2 a *refactor-to-unify*, not a build-from-nothing.)
...
- **The gap:** CLV machinery exists (schema `beliefs.clv_bps` — **integer bps, `Option<i64>`**, computed in integer arithmetic, events.rs:314-339; set-once persistence; shadow use) but is **never computed live** — weather passes `None`, funding never calls it, `price_snapshots` is **never populated**, `benchmark_at` unused. The fast gate is dark.
...
CLV is computed at resolution by SELECTING a benchmark from the rich series, per a **config policy** (decoupling: the policy is data, not a literal): default = *the last LIQUID snapshot at or before a configured pre-horizon offset* (e.g. T-1h), with fallbacks (nearest liquid within a window) if that mark is missing. `SnapshotsRepo::benchmark_price(market_id, policy) -> Option<Snapshot>`; `clv_bps = devig(benchmark) − devig(entry)` in **integer bps** (`i64`, matching `events.rs`'s integer math — the house rule keeps price math off `f64`; any `f64` is confined to the analytics/reliability-diagram layer, never the score column). The live resolution path computes CLV for every TRADED belief and persists via `resolve_and_score`.
...
- `k_unc ∈ (0,1]` **estimation-uncertainty shrinkage → 1 as resolved-count n grows** (Baker–McHale / Chu–Wu–Swartz). **Theorem: with any σ²>0, expected growth slopes down at full Kelly — full Kelly is strictly dominated, never an operating point.** `k_unc` is the formal "humility discount" the spec hand-waves; make it a diagnosable factor. (New work; small.)
...
- **OPEN (this doc's targets):** the **live ScoringRule dispatch** (D-4), **CLV-live** (§5), the **Murphy/PIT/reliability** diagnostics (§4), **per-mind keying** (D-1), **RPS for ladders + Log score** (D-5), and **automated demotion** (§10). Feedback latency: weather ~daily, funding ~8h → both reach n=50 in ~1–2 weeks (the demo window).
...
| ---------------- | ------------------------------------------------------------------------------- | --------------------------------- |
| Volume           | `n ≥` the spec §11 threshold (the shipped `go_nogo` uses `min_resolved_beliefs`; `FULL_AUTONOMY_N=50` is the calibration-autonomy ramp, NOT necessarily the GO bar — defer to spec §11/`review.rs`). Mechanical: `min_paper_days ≥ 14` | thin-data guard |
| Calibration      | Reliability error low **AND Resolution > 0** (Murphy)                           | proves informative, not base-rate |
| Edge (spec §11 semantics) | **positive CLV (prediction markets) OR positive expectancy net of modeled fees**, AND Brier beating the market-implied baseline (spec.md:384) | proper + market-relative |
| Economics        | fee/PnL ratio sane; fill-realism (maker/through-not-touch)                      | not a fee trap                    |
...
**Reuse, don't reinvent:** the GO/NO-GO verdict is ALREADY computed by `go_nogo`/`weekly_review` (review.rs:189) returning Go/NoGo/InsufficientData + reasons — these thresholds must match **spec Section 11** (spec.md:384), not a new invented bar. The architecture's job is to (a) make the gate inputs (CLV, Murphy, per-mind keying) live + correct, and (b) surface the existing verdict.

The operator dashboard surfaces, **per scope/producer/mind**: the reliability diagram (w/ bands), the Murphy split (REL/RES/UNC), rolling Brier + Log + CRPS, rolling CLV, sharpness, n, and the GO/NO-GO verdict with reasons. The Aeolus-vs-meteorologist-vs-synthesis head-to-head BECOMES a fair paired comparison once the beliefs are scored by the **same data-type-dispatched rule on the same brackets** (§3) — which is the TARGET, not today's state: today scoring is **forked per producer at the function level** (`resolve_and_score_weather_beliefs` vs `_funding_beliefs` vs the persona path), so the unified data-keyed dispatch (D-4) is a prerequisite for the head-to-head.
...
| G2  | **Unify ScoringRule dispatch (D-4)**                                            | **P1**   | scoring runs live but hand-rolled & forked per producer; one trait path enables the fair head-to-head & cheap new rules | refactor the 3 forked resolvers → one `PredictiveKind`-dispatched `rule.score(...)` in `fortuna-live`           |
...
| G7  | `**k_unc` estimation-uncertainty shrinkage (D-6)**                              | **P3**   | full Kelly is strictly dominated; formalize the humility discount                     | one factor in the sizing formula                                                                                 |

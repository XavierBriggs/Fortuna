...
- **Per-slice verifier** re-runs: the slice's TARGETED tests (the new tests — independent evidence) + **the slice's DIRECTLY-AFFECTED existing integration tests** (if the slice touches a shared hot path — the synthesis daemon path, the resolver, the gate — run THAT path's `daemon_smoke`/integration suite, not just the new unit tests) + the slice-relevant invariant/decoupling check + a mutation-spot. Do NOT re-run full DST 2000 or `clippy --all-targets` per slice. **Lesson (WS1.7):** a synthesis-path slice's scoped verify ran only its new `compose`/selection tests and MISSED a `daemon_smoke` synthesis regression (synthesis went cold). If a slice changes a shared runtime path, its scoped verify MUST run that path's existing integration tests.

...
| Forecast Feed (C 9.1) | `/forecast_feed` | `scalar_beliefs` | B (query) | **DONE** (recent half) — the recent individual forecasts newest-first with producer/event/unit, the forecast median (q=0.5 extracted in SQL), the realized outcome (null=pending → "—"), and status; the "did the vendor call it" detail. Untrusted quantiles fan + provenance NOT rendered — only the median number |

...
| Discovery — Edges (T4.5 a / item 4) | `/discovery_edges` | `market_event_edges` ⋈ `events` | B | **DONE + screenshot-verified** — the live (non-superseded) market↔event mappings JOINed to their event statement: market · venue · mapping · confidence · confirmed/proposed status · proposer/confirmer. "The markets/series UNDER the events" — the join behind the Events board's count. Runtime sqlx, newest-event-first; confirmed=green pill, proposed→honest-null confirmer; untrusted strings esc'd. Screenshot: `docs/reviews/rota-visual/rota-discovery-edges-2026-06-14.png`. Tradability join + events→edges drill-in are follow-ons |
...
| Ingest — Sources (D V2) | `/ingest_sources` | `IngestionTelemetry.sources` | B (OBS-2c) | **LIVE + OBS-3 attrs** — handler + `boardTable` + screenshot; daemon shapes it via `merge_ingest_views` (OBS-2c). Now also surfaces the source_registry admission attributes **Domains** (`domain_tags`, joined; honest null if untagged) + **Tier** (`trust_tier`) — OBS-3 populated them after OBS-2c; rendered here (`docs/reviews/rota-visual/rota-sources-health-domains-2026-06-13.png`) |
...
| Analyses (E 20.2) | `/analyses` | `domain_analyses` (+ `beliefs` fanout) | B (query) | **DONE** (browser + fanout) — artifact ledger newest-first (persona `id@version`, region, produced_at, cost in $, content-hash anchor, supersession status) + the artifact→belief FANOUT count (how many beliefs each analysis produced, via `beliefs.provenance->>'analysis_id'`). STRUCTURAL METADATA ONLY — untrusted `findings`/`signal_manifest` not exposed. The per-belief expander (the fanned-out beliefs + findings/manifest, esc'd) is a follow-on (GAPS) |

...
You are the TRACK D IMPLEMENTER (multi-track orchestration: docs/design/orchestration.md GOVERNS ownership; read it once, re-read on conflicts). An independent verification session gates everything
...
1. PRIORITY ORDER: (a) new gate findings — read the bus at the MAIN
   ...
   git rebase main); resolve conflicts only in files you own, else STOP+ledger; (b) YOUR QUEUE: news-aggregation PHASE A ONLY per the design doc
   docs/superpowers/specs/2026-06-12-news-aggregation-design.md INCLUDING
   its four-layer trust framework (s4.4) — Layers 0-2 are Phase-A binding
   (admission dossiers, structural validation, corroboration). Fixtures
   FIRST under fixtures/sources/ for every adapter; never invent feed
   behavior; missing fixture = stub + GAPS. NO model in the ingestion
   path in Phase A; Scrape/Mcp/extraction are later phases. The drive()
   seam in fortuna-live: ONE minimal flagged commit only.

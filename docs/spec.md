...
- **I5. Append-only audit log.** Every model call (prompt hash, context manifest, model id, cost), every belief, every proposal, every gate decision (pass/modify/reject plus reason), every order, every fill, every config change. Sufficient to replay any decision after the fact. Never deleted, never updated in place. **Scoped exception (C1, 2026-06-14; reconciles this invariant with 5.5):** a belief's four post-resolution SCORING columns (status, outcome, brier, clv_bps) are filled exactly once by the scoring job (5.5); the belief's decision CONTENT (p, p_raw, evidence, provenance) stays immutable and every audit/event/fill/settlement/signal row remains strictly append-only. DB-enforced — `fortuna_beliefs_guard` (belief content-immutable, scoring set-once) and `fortuna_refuse_mutation` (all other append-only stores).
...
Type-level enforcement of I1: `place` accepts only `GatedOrder`, a type constructible solely by the gate pipeline. Fee models per venue (researched 2026-06-09/11, docs/research/venue/): Kalshi event contracts taker 0.07 x p x (1-p) rounded against the trader (fixture-confirmed against a real demo fill 2026-06-11) with maker discounts and category multipliers; Kalshi perps (Kinetics) per-market maker/taker rates as decimal fractions of NOTIONAL via the venue fee_tiers endpoint ($0 launch promo as of 2026-06-11 — see the 5.15 fee-trap rule); Polymarket Intl per-category quadratic taker 0.03-0.07 with maker rebates; Polymarket US quadratic taker 0.05 / maker -0.0125 with banker's rounding. (Superseded 0.8 text claimed "Intl mostly zero" and "US flat 10bp taker" — corrected, not erased: both regimes changed in 2026.) Settlement metadata (oracle type, resolution source, expected lag) is part of `Market`, because oracle-delay artifacts must be excluded from edge scans (per the NBA microstructure finding). Fee schedules are data, not code: a versioned fee config per venue (formula type: quadratic p(1-p) coefficient | flat bps | tiered; coefficients; maker/taker variants; category multipliers; effective_date) interpreted by one fee engine, so a venue fee change is a config change with an audit row. Every fill reconciles charged fee against modeled fee; mismatch writes a discrepancy (the config is only trusted because it is continuously verified).
...
### 5.15 Perpetual-futures domain (Kinetics; operator-directed extension, 2026-06-11)

New capability, zero changes to the invariant middle: every perp order is the same sealed `GatedOrder` through the same I1 pipeline; the model remains propose-only (I6); promotion gates (I7) apply unchanged. Grounding for every venue claim: docs/research/venue/kinetics-perps-2026-06-10/ (research-loop archived); venue behavior is fixture-confirmed before any paper/live clearance, like every adapter.

**Instrument kind.** `InstrumentKind { BinaryEvent, Perp }` is threaded through `Market`, positions, and the gates. Perp positions never resolve: they carry NO settlement lifecycle (5.13 does not apply to them); in its place they carry margin/maintenance state, a mark-price feed, and funding accruals. Code paths that assume resolution (settlement watchdogs, outcome scoring) must dispatch on kind, never guess from venue.

**Price domain.** Perp prices ride in `PerpPrice`, a venue-scoped integer newtype in TEN-THOUSANDTHS of a dollar (the venue tick is $0.0001), with checked arithmetic; `rust_decimal::Decimal` only at venue payload boundaries. `Cents` remains the only money type in the event-contract core — the perps domain converts to Cents exclusively at notional/PnL/fee boundaries, with rounding always against us. A `PerpPrice` must never carry an event-contract price nor vice versa (type-level separation, not convention).

**Capital and exposure (5.14 extension).** The venue's segregated margin account maps to a DEDICATED capital envelope: perps capital is allocated explicitly at the monthly review and is never fungible with event-contract envelopes intra-month. Because API accounts are PORTFOLIO-margined (one position's excursion can consume the whole account), the margin ACCOUNT — not the position — is the exposure unit, and worst-case exposure for any perp order is the LIQUIDATION loss, never the premium analogy. The maintenance-margin formula is unpublished: the gates approximate it from recorded venue risk curves (leverage_estimates by notional) with a configured safety multiplier, and REFUSE any order whose worst case the approximation cannot bound. Funding accruals are a new periodic cash-flow entry type (append-only, reconciled against the venue's funding endpoints); I2 drawdown math includes funding paid/received and margin unrealized PnL, marked at the venue's settlement mark per the conservative-marking policy (if the venue mark and our conservative mark disagree, the worse-for-us number governs halt math).

**New gates (additive to the pipeline).** (a) Liquidation-distance floor: a configurable minimum distance between the conservative mark and the liquidation point; orders that would shrink the distance below the floor are rejected. (b) Per-asset leverage caps, config-set at or below venue maxima. (c) Funding drag: edge computations on perps subtract projected funding cost over the intended holding window. (d) Per-venue perps notional caps. All four are deterministic config-driven checks in the same pipeline; none consults the model (I1/I6).

**Venue-originated fills.** Liquidations are executed by the clearinghouse as system orders (`order_source=system`) — fills FORTUNA never placed. These are a LEGITIMATE class distinct from the orphan-fill alarm: they map to a dedicated lifecycle state, write a mandatory alert, and trigger a halt evaluation (a liquidation means the margin model was wrong; trading on a wrong margin model is not allowed to continue silently). They are never silently absorbed into position math.

**Kill switch (I4).** The kill switch gains perps coverage with its OWN credential pair: flatten = cancel-all open perp orders + `reduce_only` IOC closes (the venue requires IOC/FOK on reduce-only). Still no Postgres, no cognition-runtime dependency.

**Fee trap.** Launch-promo $0 fees are a trap for edge math: every perps edge floor evaluates at ASSUMED post-promo fees (configured; schema-example 5-12 bps of notional until the venue fee_tiers endpoint returns real rates), and all Sim results gate-re-run when real fees activate. Promo-period economics never justify a promotion (I7).

**Strategy restrictions (operator-set 2026-06-11).** funding_carry is DATA-COLLECTION-ONLY: no Sim status until >= 60 days of recorded funding history demonstrates regime persistence. perp_event_basis and funding_forecast proceed through the normal I7 ladder.

**Demo/prod divergence discipline.** The demo environment runs different tickers (suffix), different risk parameters, and a newer API build than production. Fixtures record on demo; every fixture-confirmed behavior re-verifies against production read-only endpoints before first live use; risk-parameter and fee values are always read from the TARGET environment at runtime, never baked from fixtures.

...
### Alexandria publish consume — JSONL `HistoricalSource` reader (feature/alexandria-source, 2026-06-25)
- **`AlexandriaSource` (`crates/fortuna-backtest/src/sources/alexandria.rs`):** a JSONL-backed `HistoricalSource` — the deserialize counterpart to `aeolus_archive` and its byte-parity oracle. Reads a publish directory of `beliefs/snapshots/outcomes/universe.jsonl` (each line one `serde_json::to_string` of the matching `fortuna-backtest` record type) plus an optional `trades.jsonl`/`manifest.json`. Lazy, bounded-memory line reader; does **no** time mapping and forms **no** belief (the firewall is upstream — Alexandria/ADR 0008). Consumes Alexandria's published contract (`docs/coordination/2026-06-25-fortuna-publish-handoff.md` @`f0ed506`).
- **Manifest indirection + fail-closed integrity:** when the publish dir carries `manifest.json`, the stream PATHS come from it (so the physical layout can change without touching the reader), the `schema_version` MAJOR is checked fail-closed, and `verify()` refuses a row-count mismatch. No manifest → conventional filenames (the bare four-file sample still reads). Contract handed to the Alexandria session: `docs/coordination/2026-06-25-alexandria-manifest-contract.md` (per-domain layout + `index.json`/`manifest.json` schema).
- **Paper-only invariant re-enforced at the boundary:** the optional trades stream re-checks `orders == 0` after deserialize (serde bypasses `HistoricalTrade::new`).
- **CLI dispatch:** `fortuna backtest alexandria --archive <publish-dir>` and `fortuna validate alexandria --scope … --archive <publish-dir>` (per-source dispatch in `backtest_cmd.rs`). The existing `aeolus-archive` path — `FORTUNA_WS3_ARCHIVE` fallback, read-only open, validate's no-source→`Insufficient` — is unchanged.
- **Tests:** 15 reader unit tests (sample round-trip, blank/malformed/missing-file, manifest path-indirection, schema-major refuse, row-count `verify()` incl. trades, the `orders != 0` boundary reject) + a `#[sqlx::test]` handler test proving `backtest --source alexandria` replays into the ledger and is idempotent. Independent verifier: PASS (no Critical/Important). Decoupling gate green (the `alexandria` literal stays confined to `src/sources/`).

### WS4 Demo Surface — W6b Docs/Config/Tests/Cleanup (feature/ws4-build, 2026-06-22)
- **Demo runbook:** `docs/runbooks/ws4-demo.md` — operator flow `fortuna doctor` → `backtest aeolus-archive` (seed) → `validate` → `fortuna start paper-demo` → `GET /api/rota/v1/chain?event=<linkage>`. Honesty clause: `validate` returns `Insufficient` on a thin archive (correct and expected — the gate refuses to fabricate a verdict); a real `Go`/`NoGo` requires >= 30 resolved periods.
- **Config hardened to spec §11:** `min_paper_days_mechanical` 14→30, `max_fee_pnl_ratio` 0.5→0.35, `min_resolved_beliefs_synthesis` 100→60 (spec floor; GAPS.md entry added). Prior values were laxer than spec.
- **CLV constants promoted to `[cognition]` config:** `clv_min_touch_qty` (default 1) + `clv_max_spread_cents` (default 10) are now operator-configurable in `fortuna.example.toml` and flow from `CognitionSection` → `LoopConfig` → `drive()` → `resolve_and_score_weather_beliefs_with_policy`. Prior hardcoded consts resolved (GAPS entry updated).
- **WS3 decoupling regression tests (`crates/fortuna-backtest/tests/decoupling.rs`):** three permanent `#[test]`s assert no source-name literals in `fortuna-backtest/src/` (excl. `src/sources/`) and that `fortuna-scoring/Cargo.toml` + `src/` are dependency-pure (no rand/getrandom/libm, no sqlx/tokio/async fn). Mutation-proven: planting an `"aeolus"` literal or a banned dep reds the tests.
- **mode_safe test (`crates/fortuna-cli/tests/doctor.rs`):** `doctor_mode_safe_red_on_live` asserts the mode_safe check is RED for `execution_mode=live` and GREEN for `execution_mode=paper_ledger`. Previously untested.
- **Noise→NoGo end-to-end test (`crates/fortuna-backtest/tests/validate_real_edges.rs`):** `noise_producer_yields_nogo_not_insufficient` drives a no-skill producer (p≈0.5 = baseline) through `run_sweep(LedgerEdgeProvider)` — the same real-provider path as the Go fixture — and asserts `NoGo` (not `Insufficient`, not `Go`). Closes the demo's central claim: the gate refuses noise AND passes skill, both via the same code path.
- **GAPS.md consolidation (W6b):** added entries for `events table has no ticker column` (market_ticker = market_id), `book_freshness_secs: None` deferral (rota.rs), `p_cal always Some` (UI honesty note), `min_resolved_beliefs_synthesis = 60` adoption. CLV constants GAPS entry resolved. Fixed dangling rota.rs:~2147 comment to cite the correct GAPS entry title.
- **Comment fixes:** `edge_provider.rs` Brier-loss-diff sign note reworded (code computes `baseline − model`, not `model − baseline`); `doctor.rs check_grants` `format!` table name carries a PROBE_TABLES compile-time-const safety comment.
- **ws4-live-smoke.sh:** boundary live gate for WS4 (CI-runnable); exercises `fortuna doctor --offline`, the persona/discovery co-run disjoint edge-ID test, and the persona CLV non-null-with-discovery-coactive assertion via the focused Rust integration tests.
